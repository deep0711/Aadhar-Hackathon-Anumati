    ...
    >
        <Box 
            ...
            flexDirection="row">
                ...
                { 
                    status === "Approved" ? 
                        <FontAwesome name="check-circle-o" size={42} color={colors["success"]["500"]} /> 
                        :
                        ( status === "Rejected" ? 
                            <Foundation name="x-circle" size={42} color={colors["error"]["500"]} /> 
                            : 
                            (( status === "Reviewed" || status == "Finish") ? 
                                <Ionicons name="checkmark-done-circle" size={42} color={colors["success"]["500"]} /> 
                                : 
                                <AntDesign name="exclamationcircle" size={42} color='#F98739' />
                            )
                        )
                    }
                </Box>
...
export default function Notifications({ navigation }) {
    ...
    const getConsent = async() => {
        setCount(0);
        await fetch('https://anumati.herokuapp.com/anumati-server/get-consent-to-approve', {
            method:'POST',
            headers: {
